...
class DCGAN():
    ...
    def inception_block(self, inputs, output_filters, strides=1):
    ...
    def build_generator(self):
        ...
        x = Conv2DTranspose(256,(3,3))(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = LeakyReLU(alpha=0.2)(x)

        x = self.inception_block(x, 256)

        for layer in [128,64,32]:
            padding = 'valid'
            if layer <= 64:
                padding = 'same'
            x = Conv2DTranspose(layer,(3,3),strides=(2,2), padding=padding)(x)
            x = BatchNormalization(momentum=0.8)(x)
            x = LeakyReLU(alpha=0.2)(x)

            x = self.inception_block(x, layer)
